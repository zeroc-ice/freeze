...
def getJavaLibraryPath():
    if isWin32():
        return ("-Djava.library.path=\"%s\" " % os.path.join(toplevel, "cpp", "third-party-packages",
...
def getTestEnv(lang, testdir):
    ...
    if lang in ["cpp", "java", "csharp", "python", "ruby"]:
        if isWin32():
            ...
            addPathToEnv("PATH", os.path.join(toplevel, "cpp", "bin"), env)

            pkgdir = os.path.join(toplevel, "cpp", "third-party-packages")
            pkgsubdir = os.path.join("build", "native", "bin", platform)
            platformtoolset = getPlatformToolSet()

            #
            # For Debug builds we need to add Release binaries to path to be able to use db_xxx tools
            #
